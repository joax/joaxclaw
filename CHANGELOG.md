...
- **A team started without the app now leaves a trace.** Run history was written only by JoaxClaw, so a team an agent started from Slack or a schedule was invisible afterwards: the Teams screen said *never run*, however many times it had. The gateway now records the launch itself, the moment it hands out a team's prompt — with the task it was given, so you can see what was asked for. It is recorded honestly as a launch *requested*, not a run completed: an agent can ask for a prompt and never start the team, and the screen says so rather than claiming steps that never ran. When JoaxClaw next connects it adopts the real run and takes over the record. Asking for a prompt while a team is mid-run no longer disturbs the run in progress.
...
- **Teams is one page now, not five tabs.** A team had **Build**, **Graph**, **Monitor**, **Docs** and **History**, and nothing told you which to open. Two of them edited the same team through different representations — Build changed the blueprint, Graph changed the *compiled output* of the blueprint — so the two could disagree; the tab's own tooltip warned that they would, and the file format carried a flag to record when they had. Meanwhile the thing you do most, running the team against a task, sat buried inside the editing form. The team is now shown as what it is: a numbered sequence of steps, each with its role, the agent that plays it and what it does. The task box and **Run** are the first thing on the page. A run reports its progress *on that same sequence* — the step that is working, the ones behind it, the one that failed and why — instead of in a separate tab with its own vocabulary. Branching is drawn on the step it belongs to, so a team that doesn't branch shows nothing about branching, and a review gate appears as a pause on the approach to the step it guards. Runs and saved versions moved to a rail beside it. **The Graph editor is gone from Teams**: the sequence is the team, the executable flow is generated from it, and a second editable copy could only drift — none of the teams this was built against had ever used it. It remains in Processes, which is where it belongs. The separate phone screen is gone too, so there is one layout to change instead of two that quietly diverged.

- **Reconnecting works again.** In 0.24.0 and 0.24.1, once a connection dropped — a laptop waking from sleep, a network change, a gateway restart — the app could not get back, failing every attempt with *"Auth rejected: unknown method: connect"* until it was restarted. The rejection never came from the gateway: the app had started skipping methods the gateway doesn't advertise, and the handshake itself is never advertised, while the list from the previous connection was never cleared. So the first connection worked and every one after it refused itself. Scope renegotiation and the retries for a stale device token or a gateway still starting up reopen the same way, and were broken too. Each connection now starts with a clean slate, and the handshake is never skipped.
- **A tool call the model writes out as text now says that nothing ran.** Models sometimes put a call into their reply instead of making it — `<ls><path>/home</path></ls>`, a `<tool_call>{…}</tool_call>`, a `<read-files>` list copied from the gateway's own compaction notes — and the chat showed the raw XML, after something like *"Let me check the vault…"*, with nothing having been checked. No gateway executes these. They now come out of the prose and show as **Written as text — nothing was run**, with a plain summary of each attempt and its details on request. This also covers `<bash>`, `<edit>` and similar tags, which the chat used to draw as action pills with a lightning icon, as if they had run; a text `<edit>` keeps its diff, labelled as a proposal that was not applied.
...
- **The approvals banner could never have shown anything.** Approvals shipped in 0.24.0 with a call the gateway rejects: the pending lists carry only a raw request, so each one is looked up with `approval.get` to fetch the reviewer-safe description — and that method takes the approval id and *nothing else*. The app also sent the `kind`, which its sibling `approval.resolve` does require, and against a closed schema an extra property fails the whole call. Because the lookup step treats a failure as "nothing to show", the result was silent: no error, no banner, just an agent still waiting. Caught by raising a real approval on a live gateway and watching the call come back `invalid approval.get params`. Every other method added in 0.24.0 was re-checked against its schema the same way; this was the only one wrong.
...
- **Search inside your conversations, not just their titles.** The Sessions filter box only ever matched session names and keys, and only among the sessions already loaded — so finding the conversation where something was actually said meant opening them one by one. Typing two or more characters now offers **Search messages**, which runs the gateway's full-text transcript search and lists the matching messages grouped by conversation, with the sender and time; clicking one opens that session in Chat. It stays a deliberate button rather than firing as you type, because unlike the name filter beside it every run reads transcripts on the gateway. Two things the gateway's shape forced, both found by probing it: the search is **scoped to an explicit list of sessions** rather than being global — omitting the list makes the gateway fall back to a session key called `main` and fail outright on a multi-agent setup — and it accepts at most **200** of them, so the newest 200 are searched and the results say plainly when older ones were left out.

- **Notifications now reach your phone with the app closed.** The PWA could only alert you while it was still in memory — switch away long enough for the OS to reclaim it and a finished run went unannounced. True background push was blocked on the gateway side; OpenClaw 2026.9 unblocked it, adding `push.web.*` and keeping its own VAPID signing key and browser subscriptions in `state/openclaw.sqlite`, with no config key to set up. **Settings → Notifications** gains *"Also notify me when the app is closed"*, which registers this browser with your gateway — it works because the PWA already pairs as its own device, which is exactly what `push.web.subscribe` requires. A fresh subscription starts on the gateway's defaults, which enable **approval requests and nothing else**; JoaxClaw has no approvals view yet, so it opts itself into the four categories it can actually act on instead — **agent finished**, **agent has a question**, **automation failed**, **background task failed** — each toggleable, with a **Send a test** — plus **approval needed**, once that banner landed. An agent parked waiting on your answer or your sign-off is the case most worth waking a phone for, since nothing proceeds until you reply. Only **human mentions** stay off, having no surface here at all, since a notification that opens onto nothing is worse than no notification. Tapping one routes in-app: the gateway sends its own Control UI path, so a failed automation opens Automations and a question opens the chat, and anything we have no view for simply focuses the app rather than opening the gateway's own UI over the top of this one.

### Fixed

- **The type checker was never actually running.** `npm run type-check` — also part of the pre-commit hook — pointed at a root `tsconfig.json` that is a *solution* file: `"files": []` plus two project references. Plain `tsc --noEmit` against that traverses nothing and exits 0 every time, so the gate has been passing regardless of the code. Pointed at the real projects it reported **59 errors**, all of which are now fixed, and the script checks both of them properly. Four were live bugs rather than typing noise: the Dashboard's **"sessions active" chip and Sessions tile navigated nowhere**, because Sessions is a tab inside Gateway rather than a section of its own; a **team that fails validation showed a generic "not runnable" message** instead of the reasons, since the code read a `reason` field that never existed on the result; **GPU memory always read as zero**, because the fields were named `memUsed`/`memTotal` where `systeminformation` calls them `memoryUsed`/`memoryTotal`; and an **Obsidian vault always displayed as "remote"**, local or not, because the vault reference had no `mode` to read. The rest were dead comparisons, unused imports, and typings that had drifted — including `Btn`, which never declared the `title` prop or the icon-only form that a dozen call sites already used.

- **Your profile and saved connections survive on the phone now.** The durable backup added in 0.23.2 only ever worked on the desktop: it writes `~/.joaxclaw/store.json` through an Electron IPC bridge, and in the browser build that bridge doesn't exist — so `writeLocalStore` returned immediately and **every write was silently a no-op**. The PWA kept your "About You" profile and your saved gateway connections in `localStorage` alone, which is the storage mobile browsers evict first: Android Chrome discards it under storage pressure, and iOS Safari clears it after seven days without a visit unless the app is installed to the Home Screen. The same class of loss the desktop backup was written to prevent, on the surface where it was most likely. The browser build now mirrors the same data to **IndexedDB**, using the identical shape and merge rules, so nothing above that layer changed. It also asks the browser for **persistent storage** at startup, which is what actually stops eviction — Chrome grants it automatically to an installed PWA, Safari weighs it by how much you use the app, and a refusal simply leaves things as they were. Under Electron the file still wins, because the packaged app (`file://`) and a dev build (`http://localhost:5173`) are separate origins with separate IndexedDB databases, and falling back there would quietly reopen the cross-origin split the file was added to close.
- **An agent blocked on an approval no longer just stalls.** When policy says a command or a plugin tool needs sign-off, the gateway parks the run and broadcasts an approval request. JoaxClaw asked for the `operator.approvals` scope at connect and then **never used it** — so from the app's side the run simply stopped, with nothing to say why or any way to respond; the only way through was the gateway's own Control UI or a channel. Pending approvals now appear in a banner at the top of the app, with the buttons the request itself permits. All three kinds are covered — a shell command (`exec`), a plugin tool (`plugin`), and a change to OpenClaw itself (`system-agent`) — listed on connect so anything raised while the app was closed is caught, and kept live by the `*.approval.requested` / `.resolved` broadcasts, so answering from your phone or the Control UI clears it here too. Two details worth knowing: the buttons come from the request's own `allowedDecisions` rather than being hardcoded, because they differ by kind (a system-agent change offers *Allow once* and *Deny* but never *Always allow*), and the banner shows the gateway's **reviewer-safe** description — fetched separately from the pending list, which carries the raw request — so the runtime working directory, environment and execution plan stay out of the UI by design. The banner is deliberately not dismissible: hiding it would put the run back into exactly the silent stall this fixes.
...
- **Talk no longer goes silent the moment the voice needs your agent.** Ask anything the realtime voice can't answer by itself and the call froze on "Working…" — the orb kept listening, the agent never spoke, and only hanging up recovered it. `brain: agent-consult` turns out not to mean "the gateway answers for you": the relay hands the voice provider an `openclaw_agent_consult` tool, and when the provider calls it the gateway parks the provider on a *"working"* placeholder, forwards the tool call to the client that owns the session, and **waits for that client to run the consult and submit the result**. JoaxClaw only drew the tool call in its activity feed, so the provider waited forever. It now runs the round-trip the gateway (and its own control UI) expects: `talk.client.toolCall` starts the agent run, the run's reply is read off the `chat` event stream, and `talk.session.submitToolResult` hands it back so the voice speaks it — with the agent's tool steps shown live on the activity row. Two things had to change with it: the Talk session is now created with an **explicit session key** (`agent:<id>:main`, or `main` for the default agent), because the gateway never reveals which key it picked and rejects the consult if the client sends a different one; and `openclaw_agent_control` ("how's it going?", "stop that") is answered too, via `talk.session.steer`. A consult that fails or times out now returns an error the voice can say out loud instead of silence, and hanging up mid-consult aborts the agent run it started. Protocol notes in [src/lib/TALK.md](src/lib/TALK.md).
- **Talk can now be told to always ask your agent.** There is a second way the voice went quiet, with a different fingerprint: an *empty* agent-activity feed, because the realtime model never called the consult tool at all — it just said "Let me check that for you" and stopped. Nothing obliges it to use the tool, and the gateway's fallback for that is configuration rather than client code. Talk's settings now carry **"Always ask the agent"** (`talk.realtime.consultRouting`): off, the voice model decides for itself whether to consult your agent; on, every finished spoken turn is routed to the agent and the voice speaks its answer. The header says which mode you're in, so a call that answers suspiciously fast is no longer a mystery.
- **Answering an agent's question actually answers it now.** When an agent asks something through the gateway's `ask_user` tool, the gateway opens a **question record** and parks the run on it — the run is waiting for `question.resolve`, not for your next message. JoaxClaw only ever answered by sending the chosen label as an ordinary chat turn, which is precisely the path OpenClaw 2026.9's *"recover lost answer confirmations without sending the same input through ordinary steering again"* fix describes: the parked run isn't listening to the transcript, so the answer could go nowhere while the card looked answered. Questions the gateway is holding for the open chat now appear **pinned above the composer** — a parked run isn't something to scroll back for — and clicking an option resolves the record properly. A record with several questions (the gateway allows up to three) holds until all of them are answered, because it resolves in one call. Answering from somewhere else, like the gateway's own Control UI or another device, clears the card here rather than leaving it clickable. This needed a new scope: `operator.questions`, which — unlike `operator.talk` — **is not implied by `operator.write`**, so every one of these calls was previously rejected outright. The in-transcript `<ask>` card is untouched: nothing is parked on those, and chatting the answer back is the whole mechanism. Questions asking for a **secret** are deliberately left to the gateway's own UI, since this card renders options as buttons and has no masked input.
...
- **Works with OpenClaw 2026.8 again.** The gateway's 2026.8 upgrade renamed config keys and RPC parameters the app was still sending, and the new versions reject the old names outright rather than tolerating them — so **saving subagents, toggling an automation, and skill descriptions were all broken** against an upgraded gateway. `agents.list` became `agents.entries` (a map keyed by agent id, not an array), `cron.update` takes `id` rather than `jobId`, and `skills.status` now refuses an unscoped call when several agents are configured. Reads still accept either shape, so a gateway you haven't upgraded keeps working. All 73 methods the app calls were probed against a live 2026.8.2 gateway to confirm this was shape drift rather than removals. Two gateway-side settings are still worth checking yourself if you hit them: `talk.agentId` and `agents.defaults.systemAgent.agentId` being unset break Talk and agent-less automations respectively.
- **Reconnecting no longer piles up blank turns.** The live-run re-attach added in 0.22.0 could stack empty assistant bubbles and leave the composer stuck on "model working" — reported as the hosted web app freezing to a blank screen. A dropped socket clears the streaming flag while the session stays active, so the pane re-attached on every blip and appended another placeholder, each one blocking the input until the stall detector freed it. On a flaky connection it repeated indefinitely.
- **Loaded models are reported per engine.** A gateway commonly runs a second, isolated Ollama for background work, which loads its own copy of a model into the same GPU. Five surfaces read a flattened view of the interactive instance only, so anything resident on the background one was invisible — and a compounding bug in the two model pickers made it unreachable as well.

### Changed

- **"Crons" are now "Automations"**, matching the wording the gateway itself moved to — its 2026.8 errors read *"Automation not found"*. The nav rail, mobile nav, view header, empty state, dashboard section, the Models usage column, the chat list's scheduled-run tooltip and the Help copy all follow. The cron **expression** keeps its name, because `0 9 * * *` is the name of the syntax rather than of the feature.
- **The Automations list shows the ones you created.** A live 2026.8 gateway returns 21 jobs, 13 of them generated by the gateway itself — a skill-collection review per agent, per-agent heartbeats, memory promotion — which outnumbered the 8 real ones in both the list and the dashboard. Generated jobs are identified by the marker the gateway sets on them, and hidden.

### Added

- **Syntax highlighting in the file viewer.** Code opened from the Files drawer was flat monospace; it now has highlighting and line numbers, so a file reads the same whether it arrives as a diff in chat or as a file here. No new dependency — it reuses the tokenizer the diff view already uses. Highlighting is skipped above 5,000 lines, where the file is usually a log and the cost outweighs the colour.
...
- **Tool steps you can actually read.** Expanding a step used to show the plumbing rather than what happened: a gateway restart read *"Updating action, note, reason"* above nineteen lines of protocol envelope with its payload encoded twice, and a shell script arrived as a single JSON string full of `\n` escapes. Steps now lead with the event. The header names what the call **did** — `restart`, or the first line of a script that actually does something, skipping the shebang and `set -e` preamble. The result is unwrapped and its facts shown as chips (`ok yes · pid 504495 · SIGUSR1`), with success and failure carried by colour instead of by reading `"ok": true`; JSON is now the fallback for genuine structure rather than the default. Every step shares one anatomy — Command/Request, then Result — so it reads the same whatever the tool.
- **The files your agents write are finally reachable.** When a turn writes a file, a card now appears under the message — name, type, size — and opens it in a **viewer** with four levels of zoom: inline card → side panel next to the chat → full content area → **its own window** (same mechanism as a popped-out chat, so it survives a reload). Markdown renders, code and logs get a monospace view, images/video/audio use the existing players, and anything else offers **Save a copy**, which writes to your machine through a native save dialog (a browser download in the PWA). The new **Files** panel — the folder button in the chat header — lists everything on the gateway host instead of only what chat mentioned, newest first, with a dot on files written since you last looked. This matters most on a **remote gateway**, where "I saved it to ~/report.md" used to be a dead end: the file was on the host's disk with no way to read it. Cards and the viewer work with the plugin you already have; the Files listing needs `joaxclaw-fs` ≥ **0.12.0** on your gateway (without it the panel offers the one-click install). **Read-only** by design — the models author these documents, the app shows them. Design notes: [docs/files-drawer.md](docs/files-drawer.md).
...
- **The tables models write now render as tables.** Markdown only accepts a table when the `|---|` row directly follows the header, but models routinely write one header up top and then rows grouped under section headings — which the strict reading turns into a paragraph of pipes. A 19-row test plan arrived as a single run-on line. Those rows are now re-attached to the header whose columns they match, in chat and in the file viewer alike. Untouched when a document is already well-formed. Wide tables also scroll inside their own frame instead of clipping their last columns in a narrow pane.
- **A failing panel no longer takes the whole window with it.** An error in one panel used to unmount the entire app — a blank window, with the reason visible only in DevTools. The panel now shows what went wrong and the rest of the app keeps running.
