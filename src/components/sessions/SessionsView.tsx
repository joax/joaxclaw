...
// Plain-language labels for the gateway's raw status strings.
const STATUS_LABEL: Record<string, string> = {
  running: 'Running', idle: 'Idle', done: 'Completed',
  failed: 'Failed', killed: 'Stopped', timeout: 'Timed out',
}
          ...
          <table className="w-full text-sm" style={{ borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-surface)' }}>
                ...
                <Th>Cost</Th>
                <Th onClick={() => handleSort('updatedAt')} sortKey="updatedAt" active={sort} dir={sortDir}>Last active</Th>
            ...
            <tbody>
              {filtered.map((s, i) => {
                ...
                return (
                  ...
                  >
                    <td className="px-3 py-2.5" title={s.key}>
                      <div className="flex items-center gap-1.5 min-w-0">
                        {(s.isHeartbeat || s.key.includes(':heartbeat')) && (
                          <span title="Heartbeat session" style={{ display: 'inline-flex', alignItems: 'center', flexShrink: 0 }}>
...
// Cost-first, human-readable. The token breakdown is a hover tooltip rather than
// always-on monospace arrows, so the column reads as "how much did this cost".
function CostCell({ session }: { session: Session }) {
  ...
  return (
    <span
      title={tip}
      style={{ fontSize: 12, color: cost != null && cost > 0.1 ? 'var(--warning)' : 'var(--text-secondary)' }}
    >
      {cost != null && cost > 0
        ? fmtCost(cost)
        : <span style={{ color: 'var(--text-secondary)' }}>{fmtTokens((inp ?? 0) + (out ?? 0))} tokens</span>}
    </span>
  )
}
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span
        ...
        title={ctx != null && ctx > 0 ? `${model} · ${fmtTokens(ctx)} context` : model}
      >
        {displayModel}
      </span>
    </div>
...
function LabelCell({ sessionKey, label, isEditing, onStartEdit, onSave, onCancel }: {
  ...
  return (
    <div
      className="flex items-center gap-1 group/label min-w-0"
      style={{ minHeight: 18 }}
    >
      <span className="text-sm truncate max-w-64" style={{ color: 'var(--text-primary)' }}>{label}</span>
      <button
