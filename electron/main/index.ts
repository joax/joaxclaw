...
const NATIVE_SKILLS: { slug: string; version: number; lines: string[] }[] = [
  ...
  },
  {
    ...
    lines: [
      ...
      '## Running a team',
      '',
      'Two steps. Ask the gateway for the team\'s launch prompt, then start it yourself:',
      '',
      '1. Call `teams.launchPrompt` with `{ id, task }` — `task` is what the team should do this run (it fills `{objective}`). You get back `{ prompt, controllerAgentId, name }`.',
      '2. Call `sessions_spawn` with `agentId: controllerAgentId`, `task: prompt`, and `label: "process:<id>"`.',
      '',
      'That is the whole launch. The controller then orchestrates the members itself, on the gateway — it keeps running whether or not anyone has JoaxClaw open, and the label lets the app adopt the run and show its progress when someone does.',
      '',
      'Spawn the agent the prompt names. A different one gets the Team Lead instructions with the wrong tools and model.',
      '',
      'If the user asked you to prepare a run rather than start one, call `teams.run` with `{ id, task }` instead: that queues the task for them to review and start in the Teams screen, and launches nothing.',
      '',
      '## Rules',
