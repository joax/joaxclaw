...
const NATIVE_SKILLS: { slug: string; version: number; lines: string[] }[] = [
  ...
  },
  {
    ...
    lines: [
      ...
      '2. Do NOT sit and wait. Do other useful work, or check in with `script_status({ jobId })` when it makes sense, or set a reminder (see the reminder tool) to come back after a sensible delay.',
      '3. `script_stop({ jobId })` stops a job if the user wants to cancel it.',
      '',
      ...
      '- To run a server or watcher, `script_start` it and move on; it keeps running in the background and the user can stop it from the card.',
      '- The job lives on the gateway host, so its progress keeps updating even if the app reconnects. Report the jobId to the user if they may want to follow or stop it themselves.',
      '',
    ],
  },
  {
